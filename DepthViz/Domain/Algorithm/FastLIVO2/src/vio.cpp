...
void VIOManager::updateState(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    // Accumulation vectors for parallel reduction
    std::vector<float> errors_accum(total_points, 0.0f);
    std::vector<int> n_meas_accum(total_points, 0);

    parallel_for(0, total_points, [&](size_t i) {
      ...
      visual_submap->errors[i] = patch_error;
      errors_accum[i] = patch_error;
    });

    for(int i=0; i<total_points; ++i) {
        error += errors_accum[i];
        n_meas += n_meas_accum[i];
    }
